...
class Program025
{
    ...
    {
        if (exponent< 0){//проверка на отрицателно значение, если методу прийдет значение с отрицательным значением, вызовем exception
            throw new
            ArgumentException("Exponent cannot be negative ");
        }
        if (exponent == 0||baseNumber==0) { // если baseNumber/ exponent ==0 вернем 1
            return 1;
             }
             if (baseNumber<0){
                throw new 
