...
class Program019
{
    static void Main()
    {
        ...
        bool isPalindrome2 = IsPalindromeSecond(number);// решение которое сам написал после просмотра подхода коллег)
    ...
    static bool IsPalindrome(int number)
    {
        ...
        {
            if (numberString[i] != numberString[length - 1 - i])// сравниваем первые и  последние значение, если где-то несоответствие, возвращаем false
            {
                return false;
            }
    ...
    static bool IsPalindromeSecond(int number)
    {
        ...
        for (int originNumber = number; originNumber > 0; originNumber /= 10)
        {
            ...
            int lastDigging = number % 10;

            reversNumber = reversNumber * 10 + lastDigging;
            number /= 10;
